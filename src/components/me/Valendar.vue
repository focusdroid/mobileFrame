<template>
  <div id="app">
    <div class="flex">
      <nav class="nav">
        <div class="nav-back">
          <div class="nav-back-one">&nbsp;&nbsp;&nbsp;&nbsp;</div>
          <div class="nav-back-two">
            <span class="iconfont icon-el-icon-return"></span>
          </div>
          <div class="nav-back-three"></div>
        </div>
        <div class="nav-title">体重记录</div>
        <div class="nav-date">
          <div class="nav-date-one"></div>
          <div class="nav-date-two">
            <span class="iconfont icon-el-icon-curve"></span>
            <span>日历</span></div>
          <div class="nav-date-three"></div>
        </div>
      </nav>
      <div>
...
export default {
  ...
  data(){
    return {
      calendar1:{
        ...
        select(value){ // 点击选中日期触发
          console.log(value.toString());
          console.log(value)
        },
        selectMonth(month,year){ // 左右滑触发
          console.log(year,month)
        },
        selectYear(year){ // 选年触发
          console.log(year)
        },
        timestamp:Date.now()
      },
  ...
  methods:{
    getLocalTime () { // 获取系统时间
      let date = new Date()
      this.calendar1.value.push(date.getFullYear())
      this.calendar1.value.push(date.getMonth() + 1)
      this.calendar1.value.push(date.getDate())
